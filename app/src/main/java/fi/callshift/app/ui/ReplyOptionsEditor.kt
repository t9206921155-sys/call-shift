...
class ReplyOptionsEditor(context: Context, parent: LinearLayout) {
    ...
    init {
        ...
        hint("Это пауза между ответами, а не отложенная отправка. Без НОВОГО звонка ничего повторно не отправляется. Пауза SMS может быть отдельной для SIM или общей — настройка в «SMS: защита и копия». Для остальных каналов пауза общая между SIM. Ошибки с неизвестным результатом тоже могут учитывать попытку.\n«На каждый звонок» может привести к нескольким платным SMS. Отправка в мессенджеры с пометкой РУЧНАЯ требует ваших действий. Настройка не отключает исключение «Повторный звонок — пропустить».")
        set("SMS", null, 30)
    }
